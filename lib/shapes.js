...
export class SVGShape {
    ...
    _getSVGTextAsElement(dy = 0.4) {
        return `<text x="50%" y="50%" text-anchor="middle" fill="${this.textColor}" font-size="80px" font-family="Arial" dy="${dy}em">${this.text.toUpperCase()}</text>`;
    }
}
...
export class Triangle extends SVGShape {
    ...
    render() {
        // Get the SVG container and the svg text
        const outerWrapper = this._getSVGElementOuterWrapper();
        const textSVG = this._getSVGTextAsElement(0.8);
